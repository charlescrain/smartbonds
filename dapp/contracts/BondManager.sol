contract BondManager {
    //Purpose of this contract is to store the addresses of all bonds created according to their owner
    //Others will then be able to grab the addresses so as to purchase them.
    mapping (address => uint) BondOwners;
    mapping (address => address) Bonds;
    uint numberOfOwners;
    address admin;
    ...
    function createBond(address owner, uint _par_value, uint _inv_interest, uint _frequency, uint _seconds_to_maturity) returns (int) {
        Bond bond = new Bond();
    }
}
...
contract Bond {
    ...
    function initBond(address owner, uint _par_value, uint _inv_interest, uint _frequency, uint _seconds_to_maturity) onlyowner returns (uint) {
